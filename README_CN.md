...
通过官方网站安装MindSpore后，您可以按照如下步骤进行训练和评估：
  ...
  ```shell
  # 进入脚本目录，训练SPPNET实例
  bash run_standalone_train_.sh [TRAIN_DATA_PATH] [EVAL_DATA_PATH] [DEVICE_ID] [TRAIN_MODEL]

  # 进入脚本目录，评估SPPNET实例
  bash run_standalone_eval_.sh [TEST_DATA_PATH] [CKPT_PATH] [DEVICE_ID] [TEST_MODEL]

  # 运行分布式训练实例
  bash run_distribution_.sh [RANK_TABLE_FILE] [TRAIN_DATA_PATH] [EVAL_DATA_PATH] [TRAIN_MODEL]
  ```
...
- 处理器环境运行
  ...
  bash run_distribution_.sh ./hccl.json ./imagenet_original/train ./imagenet_original/val zfnet
  ...
  bash run_distribution_.sh ./hccl.json ./imagenet_original/train ./imagenet_original/val sppnet_single
  ...
  bash run_distribution_.sh ./hccl.json ./imagenet_original/train ./imagenet_original/val sppnet_mult
  ```
...
- 处理器环境运行
  ...
  bash run_standalone_eval_.sh ./imagenet_original/val ./ckpt/best.ckpt 0 sppnet_single
  ```
...
| 参数 |  | GPU |
| -------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 资源 |  910；CPU 2.60GHz, 192核；内存：755G | GeForce RTX 3090, Intel(R) Xeon(R) Gold 6226R CPU @ 2.90GHz; Memory：256G |
